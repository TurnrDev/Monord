    ...
    async def convert(self, ctx, argument):
        try:
            if not argument.isnumeric():
                try:
                    alias = (
                        ctx.cog.session.query(models.GymAlias)
                        .filter_by(guild_id=ctx.message.channel.guild.id, title=argument.lower())
                        .one()
                    )
...
class Time(commands.Converter):
    ...
    async def convert(self, ctx, argument):
        try:
            ...
            if cleaned_start_time.isnumeric() and int(cleaned_start_time) <= max(
                utils.HATCH_TIME.total_seconds() / 60, utils.DESPAWN_TIME.total_seconds() / 60
            ):
                return datetime.datetime.utcnow().replace(tzinfo=pytz.utc) + datetime.timedelta(
                    minutes=int(cleaned_start_time)
                )

            for t_format_24h, t_format_12h in [
                ("%H:%M", "%I:%M%p"),
                ("%H%M", "%I%M%p"),
                ("%H.%M", "%I.%M:%p"),
            ]:
                ...
                if start_dt is None:
                    continue
                if (
                    start_dt - pytz.utc.localize(datetime.datetime.utcnow())
                    > utils.HATCH_TIME + utils.DESPAWN_TIME
                ):
            ...
            if argument.isnumeric():
                if not 0 < int(argument) < 6:
                    raise commands.CommandError(
                        _("{} is not a valid egg level, must be between 1 and 5").format(argument)
                    )
                return int(argument)
