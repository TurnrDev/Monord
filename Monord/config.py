...
SETTINGS = {
    ...
    "timezone": {
        ...
        "help": _(
            "Set the server (or channels) time zone. See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones for a list of time zones"
        ),
...
def set_channel_config(session, channel, key, value):
    _set_config(
        session, key, value, True, **{"guild_id": channel.guild.id, "channel_id": channel.id}
    )
